...
使用技術スタックを対象に、コミュニティ報告ベースの既知リスクを調査してから手動レビューチェックリストを生成する。

### 実行手順

**Step 1: 技術スタック確認**
`package.json` / `pyproject.toml` 等からメイン技術スタックを特定する。

**Step 2: コミュニティ調査**
`web-researcher` エージェントに以下を委譲する：
```
「<技術スタック> AI生成コードによく見られるセキュリティ上の落とし穴・アーキテクチャ上の不整合・見落とされやすいロジックエラーを調べてください。
調査対象: X(Twitter)スレッド・Stack Overflow・GitHub Issues・技術ブログ
調査結果を高リスク領域のリストとして出力してください。」
```

**Step 3: 手動レビューチェックリスト生成**
調査結果に基づき、以下の観点で高リスク領域の手動チェックリストを生成する：

- **認証・認可**: プラットフォームチャネルの検証、権限昇格の見落とし
- **ディープリンク・ルーティング**: 未検証のパラメータ経由のリダイレクト
- **ログ・エラーハンドリング**: クラッシュレポートへの機密データのロギング
- **外部入力境界**: API レスポンス・Webhook・ファイルアップロードの未検証
- **状態管理**: 競合状態・TOCTOU・キャッシュポイズニング
- **依存関係**: AI が古いバージョンのAPIを使用していないか

### チェックリスト出力フォーマット

```markdown
## AI生成コード 手動レビューチェックリスト: <技術スタック>
