...
第6原則： Bashコマンドは必ず画面に表示してください。

第7原則： ユーザー選択・判断を求める場合はAskUserQuestionツールを使うこと

第8原則： 複雑なタスクでは、TodoWriteツール等を用いてToDoリストを構造化し、進捗をユーザーと共有する。各タスクの完了時には即座に状態を更新し、可観測性を確保する。
